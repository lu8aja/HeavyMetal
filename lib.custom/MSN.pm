...
my $strVERSION = "MSN 2.0 ( +PSM )( +PP3 Auth) (08/01/2008) $REVISION";
...
sub setPSMData
{
	...
	return $self->{Notification}->setPSMData( @_ );
}
=item
...
sub do_one_loop
{
        ...
        foreach my $convo (values %{$self->getConvoList()})
        {
                $convo->{P2P}->p2pSendOne() if $convo->{P2P}->p2pWaiting;
        }
...
# Added by LU8AJA 2010-05-26
				if (exists($connection->{UbxLength}) && $connection->{UbxLength} > 0){
					if (length($connection->{buf}) >= $connection->{UbxLength}){
						my $sMsgUbx = substr($connection->{buf}, 0, $connection->{UbxLength}, '');
						$connection->{UbxLength} = 0;
						$connection->CMD_UBX_data($sMsgUbx);
					}
				}
				...
				if (!exists($connection->{UbxLength}) || $connection->{UbxLength} == 0){
	                while( $connection->{buf} =~ s/^(.*?\n)// ){
	                        ...
	                        $self->call_event( $self, 'RXDATA', $incomingdata );
